      ...
      <div class="table-container">
        <div class="row">
          ...
          <a href="usersSec"><button class="btn btn-info"
            ...
            <div class="col-md-2">
          <button class="btn btn-info"
              style="background: #558A16;" disabled>Usuarios Administrador</button>
              </div>
        </div>
        ...
        <table class="table-style table table-striped" style="width:70vw; text-align:left">
          ...
          <tbody>
            ...
            <button style="font-size:13px" class="btn btn-info" @click="deleteUser(user.id)"><i class="fa fa-trash"></i>Eliminar</button></div></td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  methods: {
    async getUsers() {
      this.users = [];
      try {
        let response = await db.collection("users").where('role', '==', 'Administrador').get().then((doc) => {
            doc.forEach((res) => {
